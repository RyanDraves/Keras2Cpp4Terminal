...
void keras::LayerConv2D::load_weights(std::ifstream &fin) {
  ...
  // cerr << "LayerConv2D " << m_kernels_cnt << "x" << m_depth << "x" << m_rows <<
...
void keras::LayerActivation::load_weights(std::ifstream &fin) {
  ...
  // cerr << "Activation type " << m_activation_type << endl;
}

void keras::LayerMaxPooling::load_weights(std::ifstream &fin) {
  fin >> m_pool_x >> m_pool_y;
  // cerr << "MaxPooling " << m_pool_x << "x" << m_pool_y << endl;
}
...
void keras::LayerDense::load_weights(std::ifstream &fin) {
  ...
  // cerr << "weights " << m_weights.size() << endl;
  fin >> tmp_char; // for '['
  for(int n = 0; n < m_neurons; ++n) {
    fin >> tmp_float;
    m_bias.push_back(tmp_float);
...
void keras::missing_activation_impl(const string &act) {
  cerr << "Activation " << act << " not defined!" << endl;
  cerr << "Please add its implementation before use." << endl;
  exit(1);
}
...
keras::DataChunk* keras::LayerDense::compute_output(keras::DataChunk* dc) {
  /*cerr << "weights: input size " << m_weights.size() << endl;
  cerr << "weights: neurons size " << m_weights[0].size() << endl;
  cerr << "bias " << m_bias.size() << endl;*/
...
float * keras::KerasModel::compute_output_wrapper(float * input, int input_size, int output_size) {
  keras::DataChunkFlat *dc_input = new DataChunkFlat(input_size);
  for (unsigned int i = 0; i < input_size; i++) {
    dc_input->f[i] = input[i];
  }
  std::vector<float> v_output = compute_output(dc_input);
  delete dc_input;
  float* ret = new float[output_size];
  for (unsigned int i = 0; i < output_size; i++) {
    ret[i] = v_output[i];
  }
  // Store the address of the float array so it can be cleaned up later
  freedom = ret;
  return ret;
}
...
std::vector<float> keras::KerasModel::compute_output(keras::DataChunk *dc) {
  ...
  for(int l = 0; l < (int)m_layers.size(); ++l) {
    if (m_verbose) cerr << "Processing layer " << m_layers[l]->get_name() << endl;
    out = m_layers[l]->compute_output(inp);

    if (m_verbose) inp->show_name();
    if (m_verbose) out->show_name();
...
void keras::KerasModel::load_weights(const string &input_fname) {
  if(m_verbose) cerr << "Reading model from " << input_fname << endl;
  ifstream fin(input_fname.c_str());
  if (!fin.is_open() && m_verbose) cerr << "Error opening file" << endl;
  string layer_type = "";
  string tmp_str = "";
  int tmp_int = 0;

  fin >> tmp_str >> m_layers_cnt;
  if(m_verbose) cerr << "Layers " << m_layers_cnt << endl;

  for(int layer = 0; layer < m_layers_cnt; ++layer) { // iterate over layers
    fin >> tmp_str >> tmp_int >> layer_type;
    if(m_verbose) cerr << "Layer " << tmp_int << " " << layer_type << endl;
    ...
    if(l == 0L) {
      if (m_verbose) cerr << "Layer is empty, maybe it is not defined? Cannot define network." << endl;
      return;
    }
    l->load_weights(fin);
  ...
  if (m_verbose) cerr << "Model initialized" << endl;
...
extern "C" {
  keras::KerasModel* NeuralNet_new(char* filename, bool verbose) { return new keras::KerasModel(string(filename), verbose); }
  float * NeuralNet_compute_output(keras::KerasModel* model, float * input, int input_size, int output_size) { return model->compute_output_wrapper(input, input_size, output_size); }
  void NeuralNet_free_memory(keras::KerasModel* model) { model->free_memory(); }
}